...
const period = computed({
  get: () => store.period,
  set: (period) => store.setPeriod(period),
});
...
<template>
  <div>
    <div class="flex gap-2 w-full flex-row items-center">
      <span class="material-symbols-outlined text-gray-500 pt-6"> event </span>
      ...
      <div class="flex flex-col w-full">
        ...
        <transition name="fade-slide">
          <span v-if="error" class="text-red-600 text-xs flex float-end">
            {{ error }}
          </span>
        </transition>
      </div>
